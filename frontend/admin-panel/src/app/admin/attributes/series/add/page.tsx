...
const AddSeries = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        await createAttribute("series", values);
        setSnackbarMessage("Series added successfully!");
        setOpenSnackbar(true);
        formik.resetForm();
      } catch (error: any) {
        setSnackbarMessage(error.message || "Error adding series.");
        setOpenSnackbar(true);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <PageContainer title="Add Series">
      <Breadcrumb title="Add Series" />
      <ParentCard title="Add New Series">
