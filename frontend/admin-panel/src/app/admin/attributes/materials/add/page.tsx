...
const AddMaterial = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        await createAttribute("materials", values);
        setSnackbarMessage("Material added successfully!");
        setOpenSnackbar(true);
        formik.resetForm();
      } catch (error: any) {
        setSnackbarMessage(error.message || "Error adding material.");
        setOpenSnackbar(true);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <PageContainer title="Add Material">
      <Breadcrumb title="Add Material" />
      <ParentCard title="Add New Material">
