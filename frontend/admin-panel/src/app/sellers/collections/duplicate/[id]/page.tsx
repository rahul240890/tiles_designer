...
import { createCollection, fetchSuitablePlaces, getAllAttributes, getCollectionById } from "../../../api/collection";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (!response.error) {
          showNotification("Collection duplicated successfully!", "success");
          router.push("/sellers/collections/list");
        } else {
          showNotification(response.error, "error");
        }
      } catch (error) {
        showNotification("Failed to duplicate collection", "error");
      }
    },
  });

  return (
    <PageContainer title="Duplicate Collection" description="Modify details before creating a duplicate">
      <ParentCard title="Create Duplicate Collection">
